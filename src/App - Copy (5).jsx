  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col font-sans selection:bg-indigo-100">
      ...
      <footer className="py-10 border-t border-slate-100 flex flex-col items-center gap-3">
        <div className="flex items-center gap-2 text-slate-900 font-black text-lg tracking-tighter">
      ...
      {activeTab === 'overview' && (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white rounded-[3rem] p-2 border border-slate-100 shadow-sm"><BiasAnalysis data={locationBias} title="Geographic Intel" subtitle="Regional Hire Velocity" xKey="Location" /></div>
            <div className="bg-white rounded-[3rem] p-2 border border-slate-100 shadow-sm"><BiasAnalysis data={experienceBias} title="Tenure Intel" subtitle="Experience Band Performance" xKey="ExperienceGroup" /></div>
          </div>
              ...
              <table className="w-full text-left">
                <thead className="bg-slate-50/50 border-b border-slate-100">
                  <tr className="text-[9px] font-black uppercase text-slate-400 tracking-widest">
                    <th className="p-5">Object</th><th className="p-5">Op</th><th className="p-5">Mutation Path</th><th className="p-5">Agent</th><th className="p-5 text-right">Timestamp</th>
                  </tr>
                </thead>
          ...
          <div className="flex flex-col lg:row justify-between items-center bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm gap-6">
            ...
            <div className="flex flex-1 max-w-3xl gap-4 w-full">
              ...
              >
                ...
                <option value="1">Level 1: Admins</option>
                <option value="2">Level 2: Recruiters</option>
                <option value="3">Level 3: Candidates</option>
              </select>
            </div>
          ...
          <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
            <table className="w-full text-left">
              <thead className="bg-slate-50/50 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                <tr><th className="px-10 py-6">Identity Signature</th><th className="px-10 py-6">Privilege Tier</th><th className="px-10 py-6">Status</th><th className="px-10 py-6 text-right">Operational Toggle</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {filteredUsers.map(u => (
        ...
        <tbody className="divide-y divide-slate-50">
          ...
          {entitySubTab === 'candidates' && candidateList.map(c => (
            <tr key={c.CandidateID} className="hover:bg-slate-50/50 group transition-colors">
              ...
              <td className="p-8 text-right text-[10px] font-black text-slate-400 italic font-mono opacity-50">
          ...
          {entitySubTab === 'skills' && skillList.map(s => (
            <tr key={s.SkillID} className="hover:bg-slate-50/50 group">
              <td className="p-8 font-mono text-[10px] text-slate-300"># {s.SkillID}</td>
                ...
                <div className="overflow-y-auto flex-1 custom-scrollbar pr-4">
                  <table className="w-full text-left">
                    <thead className="sticky top-0 bg-white border-b border-slate-50 uppercase font-black text-[9px] text-slate-300 tracking-[0.2em]">
                      <tr><th className="pb-4">Signature</th><th className="pb-4 text-right">Commit Date</th></tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
    ...
    <div className="space-y-10 relative animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl relative z-10 overflow-hidden animate-in zoom-in duration-300">
                <div className="p-8 border-b bg-slate-50 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] mt-1">Personnel Match Report • SCORE: {selectedMatch.TotalMatchScore}</p>
                    </div>
                ...
                <div className="p-8 space-y-6 max-h-[60vh] overflow-y-auto">
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="bg-blue-600 p-6 rounded-3xl text-white">
                            <p className="text-[9px] font-black text-blue-200 uppercase tracking-widest mb-1">Geographic Bonus</p>
                        ...
                        {selectedMatch.SkillsDetails?.map((s, idx) => (
                            <div key={idx} className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                ...
                                <div className="h-1.5 bg-slate-200 rounded-full relative">
                                    <div className="absolute h-full bg-slate-900 rounded-full transition-all duration-1000" style={{ width: `${s.CandidateLevel * 10}%` }}></div>
                                    <div className="absolute h-4 w-1 bg-blue-500 -top-1.5 rounded-full" style={{ left: `${s.RequiredLevel * 10}%` }}></div>
                                </div>
                            </div>
                    ...
                    {selectedMatch.StatusID === 3 && (
                        <div className="mt-8 p-8 bg-blue-50 rounded-[32px] border border-blue-100">
                            <h4 className="text-sm font-black uppercase mb-6 flex items-center gap-2 tracking-tight">
                                <Calendar size={18} className="text-blue-600" /> Interface: Schedule Session
                            </h4>
                            <form onSubmit={handleScheduleSubmit} className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-[9px] font-black text-slate-400 block mb-1 uppercase tracking-widest">Inception</label>
                                        <input type="datetime-local" required className="w-full p-3 bg-white border-2 border-transparent focus:border-blue-500 rounded-2xl text-xs font-bold outline-none transition" value={schedStart} onChange={(e) => setSchedStart(e.target.value)}/>
                                    </div>
                                    <div>
                                        <label className="text-[9px] font-black text-slate-400 block mb-1 uppercase tracking-widest">Termination</label>
                                        <input type="datetime-local" required className="w-full p-3 bg-white border-2 border-transparent focus:border-blue-500 rounded-2xl text-xs font-bold outline-none transition" value={schedEnd} onChange={(e) => setSchedEnd(e.target.value)}/>
                                    </div>
                                </div>
                                <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-2xl text-xs font-black uppercase hover:bg-blue-700 transition-all shadow-xl shadow-blue-200 tracking-widest">Transmit Interview Slot</button>
                            </form>
                        </div>
                    )}
                </div>
                ...
                <div className="p-8 bg-slate-900 border-t border-slate-800 flex justify-between items-center">
                    <div className="flex flex-col">
                        ...
                        <span className="text-sm font-black text-blue-400 uppercase tracking-tighter">{selectedMatch.StatusName || 'Applied'}</span>
                    </div>
                    <div className="flex gap-3">
                        {![4, 5].includes(selectedMatch.StatusID) && (
                            <button onClick={() => handleStatusUpdate(selectedMatch.ApplicationID, 5, 'Rejected')} className="px-6 py-3 text-[10px] font-black uppercase bg-slate-800 text-red-400 rounded-2xl hover:bg-red-500 hover:text-white transition-all tracking-widest">Terminate</button>
                        )}
                        {(selectedMatch.StatusID === 1 || !selectedMatch.StatusID) && (
                            <button onClick={() => handleStatusUpdate(selectedMatch.ApplicationID, 2, 'Screening')} className="px-6 py-3 text-[10px] font-black uppercase bg-blue-600 text-white rounded-2xl shadow-lg hover:bg-blue-500 transition-all tracking-widest">Initialize Screening</button>
                        )}
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm gap-6">
        ...
        <div className="flex gap-4">
            ...
            <button onClick={() => setShowForm(!showForm)} className="bg-slate-200 text-black px-8 py-4 rounded-2xl text-[10px] font-black uppercase flex items-center gap-2 shadow-2xl hover:bg-slate-900  transition-all hover:text-white tracking-widest">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-white p-8 rounded-[40px] border-2 border-slate-100 shadow-sm flex items-center justify-between group hover:border-blue-500 transition-all">
            ...
            <div className="w-16 h-16 bg-slate-50 rounded-3xl flex items-center justify-center text-slate-900 group-hover:bg-blue-600 group-hover:text-white transition-all"><Briefcase size={32} /></div>
        </div>
        ...
        <div className="bg-white p-8 rounded-[40px] border-2 border-slate-100 shadow-sm flex items-center justify-between group hover:border-purple-500 transition-all">
            <div className="space-y-1">
                <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">Daily Interfaces</p>
        ...
        <form onSubmit={handlePostJob} className="bg-slate-50 p-10 rounded-[48px] border-4 border-slate-900 shadow-2xl grid grid-cols-1 md:grid-cols-3 gap-10 animate-in slide-in-from-top-8 duration-500">
            <div className="md:col-span-2 space-y-6">
                <div className="space-y-1">
                    ...
                    <input placeholder="e.g. Senior Intelligence Analyst" className="w-full p-5 bg-white border-2 border-transparent focus:border-blue-500 rounded-[24px] outline-none font-bold shadow-sm transition" required value={newJob.title} onChange={e => setNewJob({...newJob, title: e.target.value})}/>
                </div>
                <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-slate-500 ml-2 tracking-widest">Mission Description</label>
                    <textarea placeholder="Outline the key objectives..." className="w-full p-5 bg-white border-2 border-transparent focus:border-blue-500 rounded-[24px] outline-none h-40 font-bold shadow-sm transition" required value={newJob.description} onChange={e => setNewJob({...newJob, description: e.target.value})}/>
                </div>
                
                <div className="pt-4">
                    <h4 className="text-[10px] font-black uppercase text-slate-900 mb-6 tracking-[0.3em] flex items-center gap-2">
                        <Settings size={14}/> Technical Requirements Matrix
                    </h4>
                    <div className="grid grid-cols-2 gap-4">
                        {availableSkills.map(skill => {
                            const req = newJob.requirements.find(r => r.skillId === skill.SkillID);
                            return (
                                <div key={skill.SkillID} className={`p-4 rounded-2xl border-2 transition-all ${req ? 'border-blue-600 bg-blue-50 scale-[0.98]' : 'bg-white border-transparent'}`}>
                                    <label className="flex items-center gap-3 cursor-pointer mb-3">
                                        <input type="checkbox" checked={!!req} onChange={() => toggleSkill(skill.SkillID)} className="w-5 h-5 rounded accent-blue-600" />
                                    ...
                                    {req && (
                                        <div className="flex items-center justify-between gap-4 pt-2 border-t border-blue-200">
                                            <div className="flex items-center gap-2">
                                                <span className="text-[9px] font-black text-blue-400">MIN LVL:</span>
                                                <input type="number" min="1" max="10" className="w-12 p-1 text-[10px] border-2 border-blue-200 rounded-lg font-black text-center" value={req.minProficiency} onChange={(e) => updateSkillReq(skill.SkillID, 'minProficiency', e.target.value)}/>
                                            </div>
                                            <label className="text-[9px] font-black flex items-center gap-2 text-blue-600">
                                                <input type="checkbox" checked={req.isMandatory} onChange={(e) => updateSkillReq(skill.SkillID, 'isMandatory', e.target.checked)} className="accent-blue-600" /> MANDATORY
                                            </label>
                                        </div>
                                    )}
                                </div>
            ...
            <div className="space-y-6">
                <div className="bg-white p-8 rounded-[32px] shadow-sm space-y-6">
                    <div>
                        <label className="text-[10px] font-black uppercase text-slate-400 block mb-2 tracking-widest">Operation Base</label>
                        <input placeholder="Location" className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 rounded-2xl outline-none font-bold transition" required value={newJob.location} onChange={e => setNewJob({...newJob, location: e.target.value})}/>
                    </div>
                    <div>
                        <label className="text-[10px] font-black uppercase text-slate-400 block mb-2 tracking-widest">Experience Floor (Years)</label>
                        <input type="number" className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 rounded-2xl outline-none font-black transition" value={newJob.minExp} onChange={e => setNewJob({...newJob, minExp: e.target.value})}/>
                    </div>
                    <div>
                        <label className="text-[10px] font-black uppercase text-slate-400 block mb-2 tracking-widest">Personnel Quota</label>
                        <input type="number" className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 rounded-2xl outline-none font-black transition" value={newJob.vacancies} onChange={e => setNewJob({...newJob, vacancies: e.target.value})}/>
                    </div>
                    <button type="submit" className="w-full bg-slate-200 text-black py-6 rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl hover:bg-slate-900 hover:text-white transition-all transform active:scale-95">Publish to Nexus</button>
                </div>
            </div>
        </form>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-[40px] shadow-xl border-2 border-slate-100 overflow-hidden">
                <table className="w-full text-left">
                <thead className="bg-slate-900 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">
                    <tr>
                        <th className="px-10 py-6">Candidate Personnel</th>
                    ...
                    {matches.length > 0 ? matches.map((m, i) => (
                        <tr key={i} className="group hover:bg-slate-50 transition-all duration-300">
                            <td className="px-10 py-8">
                                ...
                                <div className="text-[9px] font-black uppercase text-blue-500 tracking-widest mt-1 flex items-center gap-1">
                                    <div className="w-1 h-1 bg-blue-500 rounded-full"></div> {m.StatusName}
                                </div>
                            </td>
                            <td className="px-10 py-8 text-center">
                                <span className="text-2xl font-black text-slate-900 bg-slate-100 px-4 py-2 rounded-2xl group-hover:bg-blue-600 group-hover:text-white transition-all">{m.TotalMatchScore}</span>
                            </td>
                            <td className="px-10 py-8 text-right">
                                <button onClick={() => openMatchBreakdown(m)} className="bg-slate-900 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase hover:bg-blue-600 transition-all tracking-widest shadow-lg shadow-slate-200">Review Dossier</button>
                            </td>
                        </tr>
                    )) : (
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4 max-h-[700px] overflow-y-auto pr-2 custom-scrollbar">
                {(activeTab === 'active' ? myJobs : archivedJobs).map(job => (
                    <div key={job.JobID} onClick={() => setSelectedJob(job)} className={`p-8 rounded-[32px] border-4 cursor-pointer transition-all relative group ${selectedJob?.JobID === job.JobID ? 'border-slate-900 bg-white shadow-2xl translate-x-2' : 'bg-white border-transparent hover:border-slate-200'}`}>
                        ...
                        <div className="pr-10 mb-4">
                            <h4 className={`font-black text-sm uppercase tracking-tight leading-tight ${selectedJob?.JobID === job.JobID ? 'text-blue-600' : 'text-slate-900 group-hover:text-blue-500'}`}>{job.JobTitle}</h4>
                            <div className="flex gap-2 mt-2">
                                ...
                                <span className="text-[8px] px-2 py-1 bg-slate-900 text-white rounded-md font-black uppercase tracking-widest">ID: {job.JobID}</span>
                            </div>
                        </div>
...
function CandidateDashboard({ user }) {
  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto p-6 pb-20">
      ...
      {/* HEADER SECTION */}
      <header className="flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm gap-6 outline">
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <section className="lg:col-span-1 bg-white p-8 rounded-[40px] border shadow-sm space-y-8">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-[10px] text-blue-600 font-black uppercase tracking-widest">Candidate ID: #{user.UserID}</p>
            </div>
          </div>
          ...
          <div className="space-y-5">
            <div className="group">
              ...
              <input className="w-full mt-1.5 p-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl text-sm font-bold transition-all outline-none" value={profile.Location || ""} onChange={e => setProfile({...profile, Location: e.target.value})}/>
            </div>
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Total Experience (Years)</label>
              <input type="number" className="w-full mt-1.5 p-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl text-sm font-bold transition-all outline-none" value={profile.YearsOfExperience || 0} onChange={e => setProfile({...profile, YearsOfExperience: e.target.value})}/>
            </div>
            <button onClick={updateProfile} className="w-full py-4 bg-blue-600 text-white rounded-2xl text-xs font-black uppercase shadow-xl shadow-blue-600/20 hover:bg-blue-500 transition-all active:scale-[0.98]">Update Profile</button>
          </div>
          ...
          <div className="pt-6 border-t space-y-4">
              ...
              {profile.docs?.map(d => (
                <div key={d.DocumentID} className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl border border-transparent hover:border-slate-200 transition group cursor-default">
                  <div className="flex items-center gap-3 text-xs font-bold text-slate-700">
                    <div className="p-2 bg-white rounded-lg shadow-sm group-hover:text-blue-600 transition">
                        <FileText size={16}/> 
                    </div>
                    {d.DocumentName}
                  </div>
            ...
            <button onClick={uploadDoc} className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl text-[10px] font-black uppercase text-slate-400 hover:border-blue-400 hover:text-blue-500 hover:bg-blue-50 transition-all">Add New Document</button>
          </div>
        </section>

        <section className="lg:col-span-2 bg-white p-8 rounded-[40px] border shadow-sm flex flex-col">
          <div className="flex justify-between items-center mb-10">
            <h3 className="text-sm font-black uppercase tracking-widest flex items-center gap-3">
              <div className="w-3 h-3 bg-blue-600 rounded-full"></div> Professional Skills Matrix
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 flex-1">
              ...
              {profile.skills?.map(s => (
                <div key={s.SkillID} className="group">
                  <div className="flex justify-between text-[10px] font-black uppercase tracking-tighter mb-2">
                    <span className="group-hover:text-blue-600 transition">{s.SkillName}</span>
                    <span className="text-slate-400">Level {s.ProficiencyLevel} / 10</span>
                  </div>
                  <div className="h-3 bg-slate-100 rounded-full overflow-hidden p-0.5">
                    <div className="h-full bg-blue-600 rounded-full transition-all duration-1000 shadow-[0_0_10px_rgba(37,99,235,0.3)]" style={{ width: `${s.ProficiencyLevel * 10}%` }}></div>
                  </div>
                </div>
              ))}
            ...
            <div className="bg-slate-100 p-8 rounded-[35px] space-y-6 h-fit text-white shadow-2xl hover:border-blue-600 hover:shadow-2xl hover:-translate-y-1 shadow-blue-900/20">
              <div>
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-blue-400 mb-1">Skill Acquisition</h4>
                  <p className="text-xs text-slate-400 font-bold">Add or update your expertise level to improve job matching.</p>
              </div>
              <select className="w-full p-4 bg-slate-100 hover:border-blue-600 hover:shadow-2xl hover:-translate-y-1 border-none rounded-2xl text-sm font-bold text-black outline-none focus:ring-2 ring-blue-500 transition-all" value={newSkill.id} onChange={e => setNewSkill({...newSkill, id: e.target.value})}>
              ...
              <div className="space-y-3">
                 <div className="flex justify-between items-center">
                    ...
                    <span className="font-black text-xl text-blue-400">{newSkill.level}</span>
                 </div>
                 <input type="range" min="1" max="10" className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-blue-500" value={newSkill.level} onChange={e => setNewSkill({...newSkill, level: e.target.value})}/>
              </div>
              <button onClick={addSkill} className="w-full py-4 bg-blue-600 text-white rounded-2xl text-xs font-black uppercase shadow-xl shadow-blue-600/20 hover:bg-blue-500 transition-all active:scale-[0.98]">Add to Matrix</button>
            </div>
          </div>
        </section>
      </div>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
            <h2 className="text-xl font-black uppercase tracking-tight flex items-center gap-3">
                <Calendar className="text-blue-600" /> Upcoming Interviews 
            </h2>
          ...
          {interviews.length > 0 ? interviews.map((meeting) => (
            <div key={meeting.ScheduleID} className="bg-white p-6 rounded-[35px] border-2 border-transparent shadow-sm hover:shadow-xl hover:border-slate-100 transition-all group relative">
              <div className="flex justify-between items-start mb-6">
                <div className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest ${meeting.TimeStatus === 'Upcoming' ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>
                  {meeting.TimeStatus}
                </div>
              ...
              <h3 className="font-black text-lg leading-tight group-hover:text-blue-600 transition">{meeting.JobTitle}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <section className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
              <h2 className="text-xl font-black uppercase tracking-tight flex items-center gap-3">
                <Star className="text-blue-600" /> AI Recommendations
              </h2>
              <span className="text-[9px] font-black bg-blue-600 text-white px-3 py-1 rounded-lg uppercase tracking-widest">Engine v2.0</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {jobs.map(job => {
              ...
              return (
                <div key={job.JobID} className={`bg-white p-7 rounded-[40px] border-2 transition-all relative overflow-hidden group ${alreadyApplied ? 'opacity-70 bg-slate-50 border-transparent' : 'hover:border-blue-600 hover:shadow-2xl hover:-translate-y-1'}`}>
                  {job.TotalMatchScore && !alreadyApplied && (
                    <div className="absolute top-0 right-0 bg-blue-600 text-white px-5 py-2.5 rounded-bl-[25px] font-black text-[10px] tracking-tighter">
                  ...
                  <div className="mb-6">
                      <h3 className="font-black text-xl leading-tight mb-1 group-hover:text-blue-600 transition">{job.JobTitle}</h3>
                  ...
                  {inspectingJobId === job.JobID && selectedGap && (
                    <div className="mb-6 p-5 bg-slate-900 rounded-[25px] space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                      <p className="text-[9px] font-black uppercase text-blue-400 tracking-widest border-b border-slate-800 pb-2">Gap Analysis</p>
                      {selectedGap.map((skill, i) => (
                        <div key={i} className="flex flex-col gap-1.5">
                          <div className="flex items-center justify-between text-[10px] font-bold">
                            <span className={`${skill.Status === 'Missing' ? 'text-rose-400' : 'text-white'}`}>
                  ...
                  <div className="flex gap-3">
                      ...
                      {alreadyApplied ? (
                        ...
                        <button onClick={() => axios.post(`${API_BASE}/apply`, { jobId: job.JobID, userId: user.UserID }).then(refreshData)} className="flex-[2] py-4 bg-blue-600 text-white rounded-2xl font-black text-[10px] hover:bg-blue-500 transition-all uppercase tracking-widest shadow-lg shadow-blue-200 active:scale-[0.98]">Quick Apply</button>
                      )}
                  </div>
                </div>
              );
        ...
        <section className="lg:col-span-1 space-y-6">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${
                        ...
                        app.StatusName === 'Hired' ? 'bg-emerald-500' : 'bg-blue-500'
                    }`}></span>
          ...
          <div className="bg-blue-600 rounded-[40px] p-8 text-white shadow-xl shadow-blue-200 relative overflow-hidden group">
              <div className="relative z-10">
                <h4 className="font-black text-lg uppercase tracking-tight leading-tight mb-2">Need to polish your profile?</h4>
                <p className="text-xs font-bold text-blue-100 mb-6">Completing your skills matrix increases your visibility to recruiters by 40%.</p>
                <div className="h-1.5 w-full bg-blue-700 rounded-full mb-4">
                    <div className="h-full bg-white rounded-full w-2/3"></div>
                </div>
                <p className="text-[9px] font-black uppercase tracking-widest text-blue-200">Profile Completion: 65%</p>
              </div>
              <Plus className="absolute -bottom-4 -right-4 text-blue-500 w-32 h-32 opacity-20 group-hover:rotate-90 transition-transform duration-1000" />
          </div>
        </section>
      </div>
    </div>
